...
const STORAGE_KEY = 'quizTaxiMilanoErrori_v1';
let DATA = null;
...
function render(){
  ...
  list.forEach(({q,index})=>{
    ...
    title.className='question'; title.textContent=q.question; card.appendChild(title);
...
document.getElementById('resetBtn').addEventListener('click',()=>{
  if(!confirm('Vuoi azzerare solo il training dei 94 errori?')) return;
  localStorage.removeItem(STORAGE_KEY);
  state={answers:{},lastQuestion:1}; mode='all'; render();
  showStatus('Training azzerato. Il Master completo non è stato toccato.');
  window.scrollTo({top:0,behavior:'smooth'});
});
window.addEventListener('beforeunload',saveProgress);
document.addEventListener('visibilitychange',()=>{if(document.hidden)saveProgress();});

fetch('errori.json?v=1',{cache:'no-store'})
  .then(r=>r.json())
  .then(data=>{
    DATA=data; loadProgress(); render();
    showStatus('Training errori pronto.');
  })
  .catch(err=>{
